...
const RYUU_BASE: &str = "https://generator.ryuu.lol/api/download";

pub fn cache_dir(app_data_dir: &Path, app_id: &str) -> PathBuf {
    app_data_dir.join("ryuu_cache").join(app_id)
}

/// Download the zip for `app_id` from Ryuu, extract `.lua` and `.manifest`
/// files into the per-app cache dir, and return the resulting AppArchiveData
/// parsed from the lua file.
pub async fn fetch_app_data(
    ...
    if api_key.is_empty() {
        return Err("Ryuu API key is not set".into());
    }

    let url = format!("{}/{}", RYUU_BASE, app_id);
    let resp = client
        .get(&url)
    ...
    let bytes = resp
        .bytes()
        .await
        .map_err(|e| format!("Failed to read Ryuu response: {}", e))?;
    ...
    tokio::fs::create_dir_all(&out_dir)
        .await
        .map_err(|e| format!("Failed to create Ryuu cache dir: {}", e))?;
    ...
    let extracted: Vec<(String, Vec<u8>)> = {
        ...
        let mut zip = zip::ZipArchive::new(reader)
            .map_err(|e| format!("Ryuu response is not a valid zip: {}", e))?;
    ...
    let lua_content =
        lua_content.ok_or_else(|| "Ryuu zip did not contain a .lua file".to_string())?;

    let parsed = lua_parser::parse_lua_file(&lua_content)
        .map_err(|e| format!("Failed to parse Ryuu .lua: {}", e))?;
...
/// Copy a cached manifest file from the Ryuu cache into `output_dir`.
...
pub async fn copy_cached_manifest(
    ...
    if !src.exists() {
        return Err(format!("Cached Ryuu manifest not found: {}", filename));
    }
