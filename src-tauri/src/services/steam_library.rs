...
mod tests {
    ...
    #[test]
    fn round_trip_one_shortcut() {
        let node = build_shortcut_node(
            0x8000_1234,
            "Test Game",
            "\"/games/test/test.exe\"",
            "/games/test/",
        ...
        let entries = vec![("0".to_string(), node)];
        let bytes = serialize_shortcuts(&entries);
        let parsed = parse_shortcuts(&bytes).unwrap();
        assert_eq!(parsed.len(), 1);
        assert_eq!(parsed[0].0, "0");
        let VdfNode::Object(fields) = &parsed[0].1 else {
        ...
        let appid = fields
            ...
            .find(|(k, _)| k == "appid")
            .map(|(_, v)| match v {
                VdfNode::Int32(n) => *n as u32,
                _ => 0,
            })
            .unwrap();
        assert_eq!(appid, 0x8000_1234);
    }

    #[test]
    fn shortcut_appid_is_stable() {
        let a = generate_shortcut_appid("\"/games/test.exe\"", "Test Game");
        let b = generate_shortcut_appid("\"/games/test.exe\"", "Test Game");
        assert_eq!(a, b);
        assert_ne!(a & 0x8000_0000, 0, "high bit must be set");
    }

    #[test]
    fn parse_real_loginusers_format() {
        let text = "\"users\"\n{\n\t\"76561198000000001\"\n\t{\n\t\t\"AccountName\"\t\t\"alice\"\n\t\t\"PersonaName\"\t\t\"Alice\"\n\t\t\"MostRecent\"\t\t\"1\"\n\t\t\"Timestamp\"\t\t\"1234567890\"\n\t}\n}\n";
        let users = parse_text_vdf_loginusers(text);
        assert_eq!(users.len(), 1);
        assert_eq!(users[0].0, "76561198000000001");
        assert_eq!(users[0].1.get("MostRecent").unwrap(), "1");
        assert_eq!(users[0].1.get("PersonaName").unwrap(), "Alice");
    }
}
