...
mod tests {
    ...
    fn make_pe(sections: &[&str]) -> Vec<u8> {
        ...
        let mut sec_off = coff + 20;
        for name in sections {
            let bytes = name.as_bytes();
            let len = bytes.len().min(8);
            buf[sec_off..sec_off + len].copy_from_slice(&bytes[..len]);
            for j in len..8 {
                buf[sec_off + j] = 0;
            }
            sec_off += 40;
    ...
    #[test]
    fn detects_bind_section() {
        let tmp = std::env::temp_dir().join("smd-drm-test.exe");
        let bytes = make_pe(&[".text", ".bind", ".rdata"]);
        std::fs::File::create(&tmp).unwrap().write_all(&bytes).unwrap();
        assert!(has_steamstub_drm(&tmp).unwrap());
        let _ = std::fs::remove_file(&tmp);
    }

    #[test]
    fn rejects_normal_pe() {
        let tmp = std::env::temp_dir().join("smd-no-drm-test.exe");
        let bytes = make_pe(&[".text", ".rdata", ".data"]);
        std::fs::File::create(&tmp).unwrap().write_all(&bytes).unwrap();
        assert!(!has_steamstub_drm(&tmp).unwrap());
        let _ = std::fs::remove_file(&tmp);
    }
    ...
    #[test]
    fn rejects_non_pe() {
        let tmp = std::env::temp_dir().join("smd-not-pe.exe");
